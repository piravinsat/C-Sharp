        ...
        public void LoadGraphicsContent(GraphicsDevice graphicsDevice)
        {
            ...
            for (int i = 0; i < gridSize1; ++i)
            {
                vertices[index++] = new VertexPositionColor(new Vector3(
                    -halfLength, 0, i * this.gridScale - halfLength), this.gridColor);
                vertices[index++] = new VertexPositionColor(new Vector3(
                    halfLength, 0, i * this.gridScale - halfLength), this.gridColor);
                vertices[index++] = new VertexPositionColor(new Vector3(
                    i * this.gridScale - halfLength, 0, -halfLength), this.gridColor);
                vertices[index++] = new VertexPositionColor(new Vector3(
                    i * this.gridScale - halfLength, 0, halfLength), this.gridColor);
            }

            this.vertexBuffer = new VertexBuffer(device, typeof(VertexPositionColor),this.vertexCount ,
