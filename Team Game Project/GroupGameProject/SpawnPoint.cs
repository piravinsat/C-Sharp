using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Microsoft.Xna.Framework;

namespace MoonDefense
{
    class SpawnPoint : IGameObject
    {
        private Random random;
        private Vector2 center;
        private double spawnChancePerSecond;
        private DateTime lastSpawnTime;

        public SpawnPoint(int x, int y, double spawnChancePerSecond)
        {
            random = new Random();
            center = new Vector2(x, y);
            this.spawnChancePerSecond = spawnChancePerSecond;
        }

        public void Update(Game1 game)
        {
            if (DateTime.Now - lastSpawnTime >= new TimeSpan(0, 0, 1))
            {
                lastSpawnTime = DateTime.Now;
                if (random.Next() <= spawnChancePerSecond)
                {
                    // spawn mob
                    game.mobs.Add(new Mob(100, center, new Vector2(1, 1)));
                }
            }
        }
    }
}
 